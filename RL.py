...
from stock_env import Stock, Action
...
class StateProcessor():
    ...
    def __init__(self):
        ...
        with tf.variable_scope("state_processor"):
            self.input_state = tf.placeholder(shape=[400, 2, 2], dtype=tf.uint8)
...
class Estimator():
    ...
    def _build_model(self):
        ...
        self.X_pl = tf.placeholder(shape=[None, 400, 2, 4, 2], dtype=tf.uint8, name="X")
        # The TD target value
...
def deep_q_learning(sess,
    ...
    for i_episode in range(num_episodes):
        ...
        for t in itertools.count():
            ...
            action = np.random.choice(np.arange(len(action_probs)), p=action_probs)
            next_state, reward, done, _ = env.step(VALID_ACTIONS[action], profit=epsilon <= 0.3)
            next_state = state_processor.process(sess, next_state)
            next_state = np.append(state[:, :, 1:], np.expand_dims(next_state, 2), axis=2)
